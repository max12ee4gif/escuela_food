  ...
  return (
    <section>
      <div className="flex items-baseline justify-between">
        ...
        <span className="text-xs font-medium uppercase tracking-wide text-muted">{phase === "leftover" ? "Sobrantes" : phase === "early" ? "Anticipadas" : phase}</span>
      </div>
      ...
      <p className="mt-3 text-sm text-muted">{day.reserved} reservados · {day.remaining} libres · {t.platesDelivered} entregados</p>
    </section>
  );
}
...
export function ReservationList({ board, onRefresh }: { board: AdminBoard; onRefresh: () => void }) {
  ...
  return (
    <section className="flex flex-col gap-3">
      ...
      {debts.length > 0 && <div className="rounded-md border border-warn/30 bg-warn-soft px-3 py-2 text-sm text-ink">Deudas: {debts.map((d) => d.name).join(", ")}</div>}
      <ul className="flex flex-col gap-3">{rows.map((row) => <ReservationCard key={row.id} row={row} onRefresh={onRefresh} />)}</ul>
    </section>
  );
}
...
function ReservationCard({ row, onRefresh }: { row: AdminReservation; onRefresh: () => void }) {
  ...
  const payLabel: Record<PaymentStatus, string> = { pending: "Pendiente", cash: "Efectivo", online: "En línea", debt: "Deuda" };
  const delLabel: Record<DeliveryStatus, string> = { reserved: "Reservado", delivered: "Entregado", noshow: "No se presentó", cancelled: "Cancelado" };
  return (
    <li className="rounded-lg border border-line bg-raised p-4">
      ...
      <p className="mt-1 text-xs text-muted">{row.timeLabel} · {payLabel[row.paymentStatus]} · {delLabel[row.deliveryStatus]}</p>
      <div className="mt-3 grid grid-cols-2 gap-2">
        <Button size="sm" variant="leaf" disabled={busy} onClick={() => void patch({ deliveryStatus: "delivered", paymentStatus: "cash" })}>Ya llegué · cash</Button>
        <Button size="sm" variant="outline" disabled={busy} onClick={() => void patch({ paymentStatus: "online", deliveryStatus: "delivered" })}>Pagó en línea</Button>
        <Button size="sm" variant="outline" disabled={busy} onClick={() => void patch({ paymentStatus: "debt" })}>Deuda</Button>
        <Button size="sm" variant="ghost" disabled={busy} onClick={() => void patch({ deliveryStatus: "noshow" })}>No se presentó</Button>
      </div>
    </li>
  );
}
...
export function DayActions({ onRefresh }: { onRefresh: () => void }) {
  ...
  return (
    <section className="flex flex-col gap-3 border-t border-line pt-6">
      ...
      <textarea id="cancel-msg" value={message} onChange={(e) => setMessage(e.target.value)} rows={2} className="w-full rounded-md border border-line bg-raised px-4 py-3 text-base text-ink outline-none" />
      ...
      <Button variant="ghost" disabled={busy} onClick={async () => {
        ...
        try {
          ...
          toast.message(data.message || "Nadie para recordar", {
            description: data.recipients.length === 0 ? "Sin reservas activas." : `${data.recipients.length} SMS de demo (Twilio pendiente): ${data.recipients.map((r) => r.name).join(", ")}`,
          });
        } catch (err) {
          toast.error(err instanceof Error ? err.message : "No se pudo armar el aviso.");
        } finally { setBusy(false); }
      }}>Probar recordatorio de mañana</Button>
    </section>
  );
}
